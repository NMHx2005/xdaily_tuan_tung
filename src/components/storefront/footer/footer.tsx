import Link from "next/link";
import { MapPin, Phone, Mail, Clock } from "lucide-react";

const categories = [
  { name: "Ghế ăn", href: "/collections/ghe-an" },
  { name: "Ghế bar", href: "/collections/ghe-bar" },
  { name: "Bàn trà", href: "/collections/ban-tra" },
  { name: "Sofa", href: "/collections/sofa" },
  { name: "Giường ngủ", href: "/collections/giuong-ngu" },
  { name: "Bàn ăn", href: "/collections/ban-an" },
];

const support = [
  { name: "Hướng dẫn mua hàng", href: "#" },
  { name: "Chính sách đổi trả", href: "#" },
  { name: "Chính sách vận chuyển", href: "#" },
  { name: "Câu hỏi thường gặp", href: "#" },
];

const contact = [
  { icon: MapPin, text: "Số 123, Đường ABC, Quận XYZ, TP.HCM" },
  { icon: Phone, text: "0123 456 789" },
  { icon: Mail, text: "contact@xdaily.vn" },
  { icon: Clock, text: "Thứ 2 - Thứ 7: 8h00 - 18h00" },
];

export function Footer() {
  return (
    <footer className="bg-neutral-900 text-neutral-300">
      {/* Main footer */}
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        <div className="grid grid-cols-1 gap-10 sm:grid-cols-2 lg:grid-cols-4">
          {/* Column 1 — About */}
          <div>
            <Link href="/" className="font-heading text-xl font-bold text-white">
              XDAILY
            </Link>
            <p className="mt-4 text-sm leading-relaxed text-neutral-400">
              Nhà máy nội thất XDAILY — Cung cấp ghế, bàn, sofa cao cấp cho mọi không gian.
            </p>
            <div className="mt-5 flex items-center gap-3">
              <a href="#" aria-label="Facebook" className="rounded-full border border-neutral-700 p-2 transition-colors hover:border-white hover:text-white">
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </a>
              <a href="#" aria-label="Instagram" className="rounded-full border border-neutral-700 p-2 transition-colors hover:border-white hover:text-white">
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
              </a>
              <a href="#" aria-label="YouTube" className="rounded-full border border-neutral-700 p-2 transition-colors hover:border-white hover:text-white">
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
              </a>
            </div>
          </div>

          {/* Column 2 — Categories */}
          <div>
            <h4 className="mb-4 text-sm font-semibold uppercase tracking-wider text-white">
              Danh mục sản phẩm
            </h4>
            <ul className="space-y-2.5">
              {categories.map((item) => (
                <li key={item.href}>
                  <Link href={item.href} className="text-sm text-neutral-400 transition-colors hover:text-white">
                    {item.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Column 3 — Support */}
          <div>
            <h4 className="mb-4 text-sm font-semibold uppercase tracking-wider text-white">
              Hỗ trợ khách hàng
            </h4>
            <ul className="space-y-2.5">
              {support.map((item) => (
                <li key={item.name}>
                  <Link href={item.href} className="text-sm text-neutral-400 transition-colors hover:text-white">
                    {item.name}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Column 4 — Contact */}
          <div>
            <h4 className="mb-4 text-sm font-semibold uppercase tracking-wider text-white">
              Liên hệ
            </h4>
            <ul className="space-y-3">
              {contact.map((item) => (
                <li key={item.text} className="flex gap-3 text-sm text-neutral-400">
                  <item.icon className="mt-0.5 h-4 w-4 shrink-0 text-neutral-500" />
                  <span>{item.text}</span>
                </li>
              ))}
            </ul>
          </div>
        </div>
      </div>

      {/* Bottom bar */}
      <div className="border-t border-neutral-800">
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <p className="text-center text-xs text-neutral-500">
            © {new Date().getFullYear()} XDAILY. Tất cả quyền được bảo lưu.
          </p>
        </div>
      </div>
    </footer>
  );
}
